...
3. **Mark Task In Progress:** Update `plan.md` to mark the task status as `[~]` (in-progress).
...
11. **Get and Record Task Commit SHA:**
    - **Step 11.1: Update Plan:** Read the track's `plan.md`, find the line for the completed task, update its status from `[~]` to `[x]`, and append the first 7 characters of the _just-completed commit's_ commit hash.
      - Example: `[x] **Task 1.1: Data Engineering** (abc1234)`
    - **Step 11.2: Write Plan:** Write the updated content back to the track's `plan.md`.

12. **Commit Plan Update:**
    - **Action:** Stage the modified `plan.md` file from the track.
    - **Action:** Commit this change with a descriptive message (e.g., `conductor(tracks/model_training): Mark task 1.1 as complete`).

### Phase Completion Verification and Checkpointing Protocol

a track's `plan.md`. Each phase in a track plan should end with a task like:
